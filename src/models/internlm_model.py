...
class InternlmModel(ToolModel):
    ...
    def generate(
        ...
        if template == "default":
            ...
            predict = self.tokenizer.decode(output[0].tolist())
            predict = predict.split("\n")[-1]
            predict = predict.replace("<|endoftext|>", "").replace("</s>", "")
            return predict
        elif template != "default":
            output, _ = self.model.chat(self.tokenizer, prompts, history=None, **params)
            return output
        # params = self.generate_params(generate_configs)
    ...
    def load_model(self, model_path, peft_path=None, trust_remote_code=True, tensor_parallel_size=1, gpu_memory_utilization=0.25):
        '''加载模型'''
        print(model_path, peft_path, trust_remote_code)
        self.tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=trust_remote_code)
        self.model = AutoModelForCausalLM.from_pretrained(model_path, device_map="auto", trust_remote_code=trust_remote_code).eval()
        if peft_path:
