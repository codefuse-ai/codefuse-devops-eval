...
from src.datasets import ToolSummaryDataset
from .base_evalution import ToolEvalution
from .utils import rec_search_key


class ToolSummaryEvalution(ToolEvalution):
    def __init__(
        self, 
        model: ToolModel,
        dataset: ToolSummaryDataset,
        base_prompt: str = '',
    ...
    def eval_metric(self, datas):
        ...
        for data in datas:
            ...
            try:
                ...
                self.error_predicts.append({"prompt": prompt, "predict": predict, "answer": answer, "response": response, "error_message": e})
        # 
